...
from app import grabber, storage
...
async def last_comments(request):
    ...
    logging.info(f'{max_offset=} {total_limits=}')
    all_comments = await grabber.fetch_last_comments(total_limits, max_offset)

    return html_templates.TemplateResponse('index.html', {'request': request, 'comments': all_comments,

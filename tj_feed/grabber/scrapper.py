...
from typing import Iterable, Iterator, List, Optional, Tuple, Dict, Union
...
API_COMMENTS_PARAMS: Dict[str, Union[str, int]] = OrderedDict({
...
async def fetch_users_page(limit: int, offset: int, lock: asyncio.Semaphore) -> Iterable[User]:
...
async def fetch_comments_page(limit: int, offset: int, lock: asyncio.Semaphore) -> Iterable[Comment]:
...
async def fetch_comments_page_next_offset(limit: int, offset: int, lock: asyncio.Semaphore) -> Optional[int]:
    request_params = API_COMMENTS_PARAMS.copy()
    request_params['limit'] = limit
    request_params['offset'] = offset
